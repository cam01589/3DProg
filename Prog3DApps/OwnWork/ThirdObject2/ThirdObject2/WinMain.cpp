...
{
    D3DXMATRIX matRotateY[5], matView, matRotateZ[5], matProjection, matTranslate[5], matRotateX[5], D3DXMatrixScaling;
	...
	if( Rotate == FALSE )
	{
		...
		D3DXMatrixRotationZ( &matRotateZ[4], 0 );//-Time );
		D3DXMatrixRotationY( &matRotateY[4], Time + 3.14159f );
	}
	else if( Rotate == TRUE )
	{
		// create a world matrices
	...
	D3DXMatrixTranslation( &matTranslate[1], 0.0f, 4.0f, 0.0f );
	D3DXMatrixTranslation( &matTranslate[2], 0.0f, 7.0f, 0.0f );
	D3DXMatrixTranslation( &matTranslate[3], 0.0f, 10.0f, 0.0f );
	D3DXMatrixTranslation( &matTranslate[4], 0.0f, 13.0f, 0.0f );
	...
	*/
	...
	matFinal[0] = matTranslate[0] * matRotateZ[0] * matRotateY[0] * matView * matProjection;
    ...
    devcon->ClearDepthStencilView(zbuffer, D3D11_CLEAR_DEPTH, 1.0f, 0);
        ...
        devcon->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
				...
				devcon->DrawIndexed( 18, 24, 10 );

			    // draw the CUBE
				devcon->UpdateSubresource(pCBuffer, 0, 0, &matFinal[2], 0, 0);
			    // WORKING devcon->DrawIndexed( 60, 18, 15 );//18
				devcon->DrawIndexed( 60, 24, 15 );
			    ...
			    // PYRAMID
				devcon->UpdateSubresource(pCBuffer, 0, 0, &matFinal[4], 0, 0);
				devcon->DrawIndexed( 18, 24, 10 );
...
void InitGraphics()
{
	...
	////--------------------- PYRAMID
		...
		//-------------------- CUBE
	{-1.0f, 1.0f, -1.0f, D3DXCOLOR(1.0f, 0.0f, 0.0f, 1.0f)},    // vertex 0
    {1.0f, 1.0f, -1.0f, D3DXCOLOR(0.0f, 1.0f, 0.0f, 1.0f)},     // vertex 1
    {-1.0f, -1.0f, -1.0f, D3DXCOLOR(0.0f, 0.0f, 1.0f, 1.0f)},   // 2
    {1.0f, -1.0f, -1.0f, D3DXCOLOR(1.0f, 0.0f, 1.0f, 1.0f)},    // 3
    {-1.0f, 1.0f, 1.0f, D3DXCOLOR(0.0f, 1.0f, 1.0f, 1.0f)},     // ...
    ...
    bd.Usage = D3D11_USAGE_DYNAMIC;
    bd.ByteWidth = sizeof(DWORD) * ARRAYSIZE( OurIndices );
